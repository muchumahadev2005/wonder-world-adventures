...
function UserDrawer({ user, token, onClose, onPremiumGranted }: UserDrawerProps) {
  const grantMutation = useMutation({
    mutationFn: () => adminApi.grantPremium(user.id, token),
    onSuccess: () => {
      toast.success(`✨ Premium granted to ${user.name}!`);
      onPremiumGranted();
      onClose();
    },
    onError: (err: Error) => {
      ...
      >
        ...
        <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <Button
              style={{ justifyContent: "flex-start", gap: 8, background: "#fef3c7", color: "#d97706", border: "none" }}
              disabled={grantMutation.isPending}
              onClick={() => grantMutation.mutate()}
            >
              <Crown size={15} /> {grantMutation.isPending ? "Granting..." : "Grant Premium"}
            </Button>
    ...
    <AdminLayout>
      ...
      {selectedUser && (
        <UserDrawer
          ...
          onPremiumGranted={() => queryClient.invalidateQueries({ queryKey: ["admin-users"] })}
        />
      )}
    </AdminLayout>
  );
