...
const ageGroups = ["3-5", "6-8", "9-11"] as const;

const LoginPage = () => {
  const [step, setStep] = useState(0);
  const [name, setName] = useState("");
  const [ageGroup, setAgeGroup] = useState<"3-5" | "6-8" | "9-11" | "">("");
  ...
  const steps = [
    ...
    { title: "How old are you?", subtitle: "Pick your age group! 🎂" },
    ...
    { title: "Favorite character?", subtitle: "Tell us your magical hero! ✨" },
  ];
  ...
  const handleNext = async () => {
    ...
    if (step === 4 && favoriteCharacter.trim().length < 1) {
      setError("Tell us your favorite character!");
      return;
    }
    ...
    try {
      setLoading(true);
      const data = await apiFetch<{ profile: any }>("/children/me", {
        method: "POST",
        body: {
          name: name.trim(),
          ageGroup,
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  {ageGroups.map((group) => (
                    <motion.button
                      ...
                      className={`px-5 py-3 rounded-2xl text-white font-bold border-2 transition-all ${
                        ageGroup === group
                          ? "border-amber-200 bg-white/25"
